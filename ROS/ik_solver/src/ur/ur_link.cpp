...
Matrix<double,CART_NO,1> findUrVelocity(Matrix<double,POSE_QUATERN,1>& goal, Matrix<double,POSE_QUATERN,1>& curr, double vMax)
{
...
UrLink::UrLink(ros::NodeHandle& nh)          
	 : isStopped(true)
         , rs(UrState::getInstance())
{    
   // subscribe
   subJointPosition = nh.subscribe(SUB_UR_JOINT_STATE, 1, &UrLink::jointPositionCallback, this);
   subTrajPositions = nh.subscribe(SUB_TRAJECTORY, 3, &UrLink::cartArrayCallback, this);
   // publish
   pubJointTraj = nh.advertise<trajectory_msgs::JointTrajectory>(PUB_TRAJECTORY,1);
   
   msgJointTraj.header.frame_id = UR_NAME;   
   ...
   msgJointTraj.joint_names[6] = "gripper";   

   // calculator
   sns = new SNSCalculator<JOINT_NO,CART_NO>(ur10::qmax, ur10::qmin, ur10::vmax);
   // start time
   //begin = ros::Time::now();
...
void UrLink::cartArrayCallback(const ik_solver::PoseStateArray& pa)
{
   if(pa.robot != UR_NAME) return;
...
void UrLink::jointPositionCallback(const iiwa_msgs::JointPosition& jp)
{
...
bool UrLink::evalAndPublish() 
{
   ...
   Matrix<double,POSE_QUATERN,1> nextPose = ur10::cartesianState(nextState);
   ...
   //std::cout << "No of points " << rows << std::endl;
   ...
   for(int i = 0; i < rows; i++) {
      ...
      int counter = 0;
      while(true) {
         Matrix<double,CART_NO,1> velocity = findUrVelocity(goalPosition, nextPose, rs.cartVMax);         
         
         Matrix<double,JOINT_NO,1> diff = sns->jointVelocity(velocity, nsv, nextState, 
                                                             ur10::Jacobian(nextState), rs.T);         
         if(!sns->successfull()) {
	    ...
	    return false;
         ...
         nextPose = ur10::cartesianState(nextState);
         
         //std::cout << nextPose.transpose() << std::endl;
